...
$script - A helpful CLI for wrapping commands
    ...
    bootstrap
        ...
        kafka nodes, and creates a topic 'fawcarTopic'. 
    down
...
case $cmd in
    bootstrap)
        ...
        docker_compose scale kafka=3
        sleep 3  # wait for kafka to start.
        echo 'bash $KAFKA_HOME/bin/kafka-topics.sh --create --topic log-collect --partitions 2 --replication-factor 2 --zookeeper zookeeper:2181' \
             | docker run --net=$projectname\_default -e RUN_TYPE=manual -a stdin -i $kafka_image &> /dev/null
        echo "Wrote topic 'log-collect'"
        echo "Bootstrap run successfully!"
        set +e
        ;;
